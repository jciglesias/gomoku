...
def greedy_best_first(board, board_size, win_len, heuristic, player, depth_limit=10):
    ...
    for m in moves:
        ...
        h = heuristic(board_size, win_len, board, player, m[0], m[1])
        mv = move(m)
        mv.heuristic = h
...
def minmax(board, player, opponent, depth, board_size, win_len, heuristic):
    ...
    for m in moves:
        ...
        score = heuristic(board_size, win_len, board, player, m[0], m[1])
    ...
    for _, move in scored[:3]:  # Only try top 3 moves for speed
        ...
        for om in opp_moves:
            ...
            opp_score = heuristic(board_size, win_len, board, opponent, om[0], om[1])
