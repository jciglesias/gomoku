...
def heuristic_row(board_size, win_len, tab, player):
    i = 0
    result = 0
...
def heuristic_diag(board_size, win_len, tab, player):
    result = 0
    j =  1 - board_size
    while j < board_size:
        i = max(0, j)
...
def heuristic(board_size, win_len, tab, player):
    tab = np.array(tab)
    res = heuristic_row(board_size, win_len, tab, player)
    res += heuristic_row(board_size, win_len, tab.transpose(), player)
    res += heuristic_diag(board_size, win_len, tab, player)
    res += heuristic_diag(board_size, win_len, np.rot90(tab, k = 1), player)
    return res